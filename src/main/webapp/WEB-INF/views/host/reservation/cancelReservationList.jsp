...
%> 
<c:set var="cancelReservationList" value = "${cancelReservationList}" />
<c:set var="contextPath"  value="${pageContext.request.contextPath}"  />
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>취소및 환불 목록</title>
<style>
@charset "utf-8";
table {
				...
				<div id="host_notice-list">
					<div class="host_contai">
						...
						<table class="board-table">
							...
							<tbody>
								<c:choose>
									<c:when test="${empty cancelReservationList}">
										<tr>
											<td colspan="9">
												<p>
													<b><span>취소 및 환불 목록이 없습니다.</span></b>
												</p>
											</td>
										</tr>
									</c:when> 
									<c:when test="${!empty cancelReservationList}">
										<c:forEach var="list" items="${cancelReservationList}">
											<tr>
												...
												<td id="cancel">
												<c:choose>	
												 <c:when test="${list.reservation_state=='n' }">승인 거부</c:when>		
												 </c:choose>										
												</td>
