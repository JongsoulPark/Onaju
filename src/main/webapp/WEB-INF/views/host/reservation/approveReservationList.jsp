...
%> 
<c:set var="approveReservationList" value = "${approveReservationList}" />
<c:set var="contextPath"  value="${pageContext.request.contextPath}"  />
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>승인 완료 목록</title>
<style>
@charset "utf-8";
table {
				...
				<div id="host_notice-list">
					<div class="host_contai">
						...
						<table class="board-table">
							...
							<tbody>
								<c:choose>
									<c:when test="${empty approveReservationList}">
										<tr>
											<td colspan="9">
												<p>
													<b><span>승인 목록이 없습니다.</span></b>
												</p>
											</td>
										</tr>
									</c:when> 
									<c:when test="${!empty approveReservationList}">
										<c:forEach var="list" items="${approveReservationList}">
											<tr>
												...
												<td id="cancel">
												<c:choose>	
												 <c:when test="${list.reservation_state=='y' }">승인</c:when>		
												 </c:choose>										
												</td>
