...
<div>
	<form id="searchVO" action="${contextPath }/admin/hostInfoList.do" method="get">
		...
		<div class="host-search-1">
			...
			<div class="host-row-col3"><div><select name="info_del_yn"><option value="">상태</option><option value="N" <c:out value="${pageMaker.cri.info_del_yn eq 'N'?'selected':'' }" />>등록완료</option><option value="Y" <c:out value="${pageMaker.cri.info_del_yn eq 'Y'?'selected':'' }" />>등록삭제</option></select></div></div>
	...
	<c:choose>
  	<c:when test="${empty hostInfoList}" >
    	<tr  height="10">
    		<td colspan="8">
    			<p align="center">
        			<b><span style="font-size:9pt;">등록된 가맹점이 없습니다.</span></b>
        		</p>
      		</td>  
    	</tr>
  	</c:when>
...
<section>
	...
	<form id="actionForm" action="${contextPath }/admin/hostInfoList.do" method="get">
			...
			<input type="hidden" name="info_del_yn" value='<c:out value="${pageMaker.cri.info_del_yn }"/>'>
