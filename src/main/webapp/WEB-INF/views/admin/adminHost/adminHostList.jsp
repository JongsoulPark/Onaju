...
<div>
	<form id="searchVO" action="${contextPath }/admin/hostList.do" method="get">
		...
		<div class="host-search-1">
			...
			<div class="host-row-col3"><div><input type="text" name="h_sellerNum2" value='<c:out value="${pageMaker.cri.h_sellerNum2 }" />' /></div></div>
		</div>
		<div class="host-search-1">
			...
			<div class="host-row-col4"><div><input type="text" name="h_name2" value='<c:out value="${pageMaker.cri.h_name2 }" />' /></div></div>
		</div>
		...
		<c:choose>
  			...
  			<c:when test="${!empty hostList}" >
    			<c:forEach  var="host" items="${hostList }" varStatus="hostNum" >
    				<tr style="cursor: pointer;" class="move" href='<c:out value="${host.h_id }" />' >
						<td>${host.rn}</td>
						<td>${host.joinDate }</td>
						<td>${host.h_id}</td>
						<td>${host.h_name }</td>   
